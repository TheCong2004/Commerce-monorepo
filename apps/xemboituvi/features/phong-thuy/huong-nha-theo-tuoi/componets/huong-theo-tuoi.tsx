...
import { Compass, Star, MoveRight, User } from "lucide-react";
...
export default function HuongNhaTheoTuoi() {
  ...
  return (
    <main className="min-h-screen bg-[#0C0D0F] py-8 md:py-12 relative overflow-hidden font-sans">
      {/* Hiệu ứng hào quang nền vàng kim (Thổ khí) */}
      ...
      <div className="max-w-7xl mx-auto px-4 mt-12 md:mt-24 relative z-10">
        <div className="flex flex-col lg:flex-row gap-10 items-start justify-center">
          ...
          {/* MENU TRÁI - Hiệu ứng trượt lần lượt từ trái sang */}
            ...
            {menuItems.map((item, idx) => (
              <FadeIn key={idx} direction="right" delay={idx * 0.1}>
                <div className="bg-[#FDFBF7] text-[#8A0000] px-6 py-4 rounded-sm text-xs font-black uppercase tracking-widest border-l-[6px] border-[#8A0000] shadow-lg hover:translate-x-2 transition-all cursor-pointer group">
                  <span className="flex items-center gap-3 group-hover:text-[#D4AF37] transition-colors">
          ...
          {/* FORM NHẬP LIỆU (TỜ SỚ) - Hiệu ứng nở nhẹ từ tâm */}
          <div className="flex-1 max-w-2xl w-full">
            <FadeIn scale={0.96} direction="up">
              <div className="relative bg-[#F3E3BC] shadow-[0_35px_60px_-15px_rgba(0,0,0,0.5)] border-x-[14px] border-[#E6D0A8] pt-12 pb-16 px-6 md:px-16 rounded-sm">
                ...
                <div className="relative z-10 text-center">
                  ...
                  <p className="text-[#252525]/60 text-sm md:text-base italic mb-10 leading-relaxed px-4">
                    Cung cấp năm sinh và giới tính để tính toán phong thủy phù hợp.
                  </p>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        {["Nam", "Nữ"].map((g) => (
                          <button 
                            ...
                            className={`relative py-6 rounded-2xl border-2 transition-all font-black uppercase tracking-widest active:scale-95 ${
                              formData.gioiTinh === g 
          ...
          {/* LA BÀN TRANG TRÍ - Hiệu ứng hiện hình mượt mà */}
          <div className="hidden lg:flex flex-col gap-8 w-1/4 items-center">
            <FadeIn direction="left" delay={0.4}>
              <div className="w-56 h-56 border-4 border-double border-[#8A0000]/40 rounded-full flex items-center justify-center relative bg-[#FDFBF7] shadow-2xl">
                ...
                <div className="absolute inset-0 rounded-full shadow-[inset_0_0_20px_rgba(212,175,55,0.1)]"></div>
              </div>
            </FadeIn>
            <FadeIn direction="up" delay={0.6}>
              <p className="text-[#D4AF37] font-black uppercase text-[11px] tracking-[0.6em] text-center leading-[2.5]">
                KHAI MÔN LẬP HƯỚNG<br/>VẬN THẾ HANH THÔNG
              </p>
            </FadeIn>
          </div>
        </div>
        ...
        {result && (
          <FadeIn direction="up">
            <div className="mt-20">
              <HouseResultView result={result} namSinh={formData.namSinh} />
            </div>
          </FadeIn>
        )}
      </div>
